/* chiehnengwu.dev — homepage renderer.
   Reads data/links.json (curated) and data/feeds.json (refreshed by GitHub Actions). */

(function () {
  "use strict";

  document.getElementById("year").textContent = new Date().getFullYear();

  var ICONS = {
    youtube: '<path d="M23 12s0-3.6-.46-5.33a2.78 2.78 0 0 0-1.94-1.94C18.88 4.25 12 4.25 12 4.25s-6.88 0-8.6.48A2.78 2.78 0 0 0 1.46 6.67C1 8.4 1 12 1 12s0 3.6.46 5.33a2.78 2.78 0 0 0 1.94 1.94c1.72.48 8.6.48 8.6.48s6.88 0 8.6-.48a2.78 2.78 0 0 0 1.94-1.94C23 15.6 23 12 23 12z" fill="currentColor"/><path d="m10 15.5 5-3.5-5-3.5z" fill="var(--bg)"/>',
    instagram: '<rect x="3" y="3" width="18" height="18" rx="5" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="4" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="17.2" cy="6.8" r="1.2" fill="currentColor"/>',
    threads: '<path d="M16.3 11.4c-.1-.05-.2-.1-.3-.14-.17-3.16-1.9-4.97-4.8-4.99h-.04c-1.74 0-3.18.74-4.07 2.09l1.6 1.1c.66-1 1.7-1.22 2.47-1.22h.03c.96.01 1.68.29 2.15.83.34.4.57.94.68 1.62a12.4 12.4 0 0 0-2.76-.13c-2.78.16-4.57 1.78-4.45 4.03.06 1.14.63 2.12 1.6 2.76.83.54 1.89.8 3 .75 1.46-.08 2.6-.64 3.4-1.66.6-.77.98-1.77 1.15-3.03.7.42 1.21.98 1.5 1.65.48 1.14.51 3.01-1.01 4.53-1.34 1.33-2.94 1.91-5.36 1.93-2.68-.02-4.7-.88-6.02-2.55C4.83 15.9 4.19 13.87 4.17 12c.02-1.87.66-3.9 1.9-5.47C7.39 4.86 9.42 4 12.1 3.98c2.7.02 4.76.89 6.13 2.57.67.83 1.18 1.87 1.51 3.08l1.9-.5c-.4-1.5-1.04-2.79-1.9-3.85C18.01 3.13 15.4 2 12.1 2h-.02C8.8 2.01 6.2 3.14 4.5 5.3 3 7.22 2.2 9.9 2.18 11.99v.02c.02 2.1.82 4.77 2.32 6.69C6.2 20.86 8.8 21.99 12.08 22h.02c2.92-.02 4.98-.78 6.68-2.47 2.22-2.22 2.16-5 1.43-6.71-.53-1.23-1.53-2.23-2.9-2.9zm-4.5 5.02c-1.23.07-2.5-.48-2.57-1.63-.05-.85.6-1.8 2.64-1.92.23-.01.46-.02.68-.02.74 0 1.44.07 2.07.21-.24 2.94-1.61 3.3-2.82 3.36z" fill="currentColor"/>',
    github: '<path d="M12 2a10 10 0 0 0-3.16 19.49c.5.09.68-.22.68-.48l-.01-1.7c-2.78.6-3.37-1.34-3.37-1.34-.45-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.61.07-.61 1 .07 1.53 1.03 1.53 1.03.89 1.53 2.34 1.09 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.56-1.11-4.56-4.94 0-1.09.39-1.98 1.03-2.68-.1-.25-.45-1.27.1-2.64 0 0 .84-.27 2.75 1.02a9.5 9.5 0 0 1 5 0c1.91-1.29 2.75-1.02 2.75-1.02.55 1.37.2 2.39.1 2.64.64.7 1.03 1.59 1.03 2.68 0 3.84-2.34 4.69-4.57 4.94.36.31.68.92.68 1.85l-.01 2.75c0 .27.18.58.69.48A10 10 0 0 0 12 2z" fill="currentColor"/>',
    linkedin: '<rect x="3" y="3" width="18" height="18" rx="3" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M7.2 10.2v6.3M7.2 7.6v.1" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/><path d="M11 16.5v-3.6a2.3 2.3 0 0 1 4.6 0v3.6M11 16.5v-6.3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" fill="none"/>',
    email: '<rect x="2.8" y="5" width="18.4" height="14" rx="2.5" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="m3.5 7 8.5 6 8.5-6" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>',
    link: '<path d="M10 13a4 4 0 0 0 5.66 0l3-3a4 4 0 1 0-5.66-5.66l-1.5 1.5M14 11a4 4 0 0 0-5.66 0l-3 3A4 4 0 0 0 11 19.66l1.5-1.5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>'
  };

  function icon(name) {
    return '<svg viewBox="0 0 24 24" aria-hidden="true">' + (ICONS[name] || ICONS.link) + "</svg>";
  }

  function el(tag, className, text) {
    var node = document.createElement(tag);
    if (className) node.className = className;
    if (text != null) node.textContent = text;
    return node;
  }

  function isExternal(url) {
    return /^https?:\/\//i.test(url);
  }

  function formatDate(iso) {
    var d = new Date(iso);
    if (isNaN(d)) return "";
    var now = new Date();
    var days = Math.floor((now - d) / 86400000);
    if (days < 1) return "今天";
    if (days < 7) return days + " 天前";
    return d.toLocaleDateString("zh-TW", { year: "numeric", month: "numeric", day: "numeric" });
  }

  /* ---------- profile ---------- */

  function renderProfile(p) {
    if (!p) return;
    if (p.name) {
      document.getElementById("profile-name").textContent = p.name;
      document.title = p.name + (p.altName ? " · " + p.altName.split(" ")[0] : "");
    }
    if (p.altName) document.getElementById("profile-alt").textContent = p.altName;
    if (p.tagline) document.getElementById("profile-tagline").textContent = p.tagline;
    if (p.bio) document.getElementById("profile-bio").textContent = p.bio;
    if (p.avatar) {
      var img = document.getElementById("avatar");
      img.src = p.avatar;
      img.alt = p.name || "";
    }
  }

  /* ---------- socials ---------- */

  function renderSocials(socials) {
    var row = document.getElementById("social-row");
    var list = (socials || []).filter(function (s) { return s.enabled !== false && s.url; });
    if (!list.length) { row.remove(); return; }

    list.forEach(function (s) {
      var a = el("a", "social-link");
      a.href = s.url;
      if (isExternal(s.url)) { a.target = "_blank"; a.rel = "noopener"; }
      a.innerHTML = icon(s.platform);
      a.appendChild(el("span", null, s.label || s.platform));
      if (s.handle) a.appendChild(el("span", "social-handle", s.handle));
      row.appendChild(a);
    });
  }

  /* ---------- projects ---------- */

  function renderProjects(projects) {
    var grid = document.getElementById("project-grid");
    if (!projects || !projects.length) {
      document.getElementById("work").remove();
      return;
    }
    projects.forEach(function (p) {
      var card = el(p.url ? "a" : "div", "project-card");
      if (p.url) {
        card.href = p.url;
        if (isExternal(p.url)) { card.target = "_blank"; card.rel = "noopener"; }
      }
      card.appendChild(el("h3", null, p.title));
      if (p.description) card.appendChild(el("p", null, p.description));
      if (p.tags && p.tags.length) {
        var ul = el("ul", "project-tags");
        p.tags.forEach(function (t) { ul.appendChild(el("li", null, t)); });
        card.appendChild(ul);
      }
      grid.appendChild(card);
    });
  }

  /* ---------- articles ---------- */

  function renderArticles(articles) {
    var wrap = document.getElementById("article-list");
    if (!articles || !articles.length) {
      document.getElementById("articles").remove();
      return;
    }
    articles.forEach(function (a) {
      var item = el("a", "article-item");
      item.href = a.url;
      if (isExternal(a.url)) { item.target = "_blank"; item.rel = "noopener"; }
      item.appendChild(el("span", "article-date", a.date ? formatDate(a.date) : ""));
      var body = el("span");
      body.appendChild(el("span", "article-title", a.title));
      if (a.source) body.appendChild(el("span", "article-source", "  " + a.source));
      item.appendChild(body);
      wrap.appendChild(item);
    });
  }

  /* ---------- Instagram oEmbed (manual, no API token needed) ----------
     Renders the official IG embed widget for hand-picked post URLs.
     Independent of the auto-fetched feed (data/feeds.json): once an
     INSTAGRAM_TOKEN is configured, real posts start appearing there too,
     and this section can keep serving as a pinned/curated list or be
     emptied out — no code change needed either way. */

  function loadInstagramEmbedScript() {
    if (window.instgrm) { window.instgrm.Embeds.process(); return; }
    if (document.getElementById("ig-embed-script")) return;
    var s = document.createElement("script");
    s.id = "ig-embed-script";
    s.async = true;
    s.src = "https://www.instagram.com/embed.js";
    document.body.appendChild(s);
  }

  function renderInstagramEmbeds(urls) {
    var section = document.getElementById("ig-embeds");
    var list = (urls || []).filter(Boolean);
    if (!list.length) { section.remove(); return; }

    var wrap = document.getElementById("ig-embed-list");
    list.forEach(function (url) {
      var card = el("div", "ig-embed-card");
      var bq = document.createElement("blockquote");
      bq.className = "instagram-media";
      bq.setAttribute("data-instgrm-permalink", url);
      bq.setAttribute("data-instgrm-version", "14");
      var fallback = el("a", null, "在 Instagram 查看這則貼文");
      fallback.href = url;
      fallback.target = "_blank";
      fallback.rel = "noopener";
      bq.appendChild(fallback);
      card.appendChild(bq);
      wrap.appendChild(card);
    });

    loadInstagramEmbedScript();
  }

  /* ---------- feed ---------- */

  function renderFeed(feed, socials) {
    var grid = document.getElementById("feed-grid");
    var items = (feed && feed.items) || [];
    grid.innerHTML = "";

    if (!items.length) {
      var enabled = (socials || []).filter(function (s) {
        return s.enabled !== false && ["youtube", "instagram", "threads"].indexOf(s.platform) > -1;
      });
      var msg = enabled.length
        ? "還沒有抓到內容。GitHub Actions 每 6 小時會更新一次。"
        : "社群帳號尚未設定 — 編輯 data/links.json 並在 repo 加上 API token 後,這裡會自動顯示最新貼文。";
      grid.appendChild(el("p", "feed-empty", msg));
      return;
    }

    if (feed.updatedAt) {
      document.getElementById("feed-updated").textContent = "更新於 " + formatDate(feed.updatedAt);
    }

    items.forEach(function (item) {
      var card = el("a", "feed-card");
      card.href = item.url;
      card.target = "_blank";
      card.rel = "noopener";

      if (item.thumbnail) {
        var img = el("img", "feed-thumb");
        img.src = item.thumbnail;
        img.alt = "";
        img.loading = "lazy";
        img.addEventListener("error", function () {
          var fb = el("div", "feed-thumb-fallback");
          fb.innerHTML = icon(item.platform);
          img.replaceWith(fb);
        });
        card.appendChild(img);
      } else {
        var fallback = el("div", "feed-thumb-fallback");
        fallback.innerHTML = icon(item.platform);
        card.appendChild(fallback);
      }

      var body = el("div", "feed-body");
      var meta = el("div", "feed-meta");
      var badge = el("span", "platform-badge");
      badge.innerHTML = icon(item.platform) + "<span>" + (item.platform || "") + "</span>";
      meta.appendChild(badge);
      if (item.date) meta.appendChild(el("span", "feed-date", formatDate(item.date)));
      body.appendChild(meta);
      body.appendChild(el("p", "feed-text", item.title || item.text || ""));
      card.appendChild(body);

      grid.appendChild(card);
    });
  }

  /* ---------- boot ---------- */

  function loadJSON(path) {
    return fetch(path, { cache: "no-cache" })
      .then(function (r) { return r.ok ? r.json() : null; })
      .catch(function () { return null; });
  }

  Promise.all([loadJSON("data/links.json"), loadJSON("data/feeds.json")])
    .then(function (results) {
      var links = results[0] || {};
      var feed = results[1] || { items: [] };
      renderProfile(links.profile);
      renderSocials(links.socials);
      renderProjects(links.projects);
      renderInstagramEmbeds(links.instagramEmbeds);
      renderArticles(links.articles);
      renderFeed(feed, links.socials);
    });
})();
